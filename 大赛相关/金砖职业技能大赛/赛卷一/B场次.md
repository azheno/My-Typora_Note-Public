...
为了方便公司企业内员工交流以及打造宽松化的办公环境，公司决定在企业内部搭建一个开源的博客系统方便员工进行网上交流以及存储相关技术资料，技术部门根据公司的安排决定采用k8s编排部署wordpress的博客系统，同时将博客数据进行持久化存储。



### 1.上传镜像到dockerhub并使用generic模式创建secret。

上传mysql-5.6镜像以及wordpress镜像到dockerhub仓库中并设置标签为latest。在k8s集群中使用literal格式创建secert名称为mysql-pass键值对为password=000000。



请将创建secert的命令提交到答题框【2分】



评分检测：kubectl create secret generic mysql-pass --from-literal=password=000000
...
```shell
kubectl create secret generic mysql-pass --from-literal=password=000000
[root@master wordpress]# kubectl describe secret mysql-pass
Name:         mysql-pass
Namespace:    default
Labels:       <none>
Annotations:  <none>
...
### 2.创建持久化存储PersistentVolume。

编写名为db_pv.yaml的yaml文件为mysql创建PersistentVolume要求:mysql挂载位置为/var/lib/mysql容量大小为3GB存储类型为本地存储，名称为mysql-persistent-storage访问模式为ReadWriteOnce。



请将db_pv.yaml中的内容提交到答题框。【1分】



评分检测：kind: PersistentVolume&& storage: 3Gi&& path: "/var/lib/mysql"
...
[root@master ~]# mkdir -p /var/www/html
 [root@master wordpress]# cat db_pv.yaml 
apiVersion: v1
...
### 3. 编写名为word_pvc.yaml的yaml文件为wordpress创建PersistentVolume

要求：wordpress挂载位置为/var/www/html名称为word-pv存储空间为3GB访问模式为ReadWriteOnce。



请将word_pvc.yaml的内容提交到答题框。【1分】



评分检测：name: word-pv&& storage: 3Gi&& path: "/var/www/html"
...
```yaml
[root@master wordpress]# cat word_pvc.yaml 
apiVersion: v1
kind: PersistentVolume
metadata:
  name: word-pv
...
编写名为mysql-deployment.yaml 的yaml文件创建数据库service要求名称为wordpress-mysql标签为wordpress开放端口为3306 clusterIP为none,同时创建PersistentVolumeClaim名称为mysql-pv-claim大小为2GB读写模式同时创建Deployment名称为wordpress-mysql使用mysql:5.6镜像同时指定密码从名称为mysql-pass的secret获取，并将/var/lib/mysql挂载到mysql-persistent-storage中。



请将mysql-deployment.yaml中的内容提交到答题框。【4分】



评分检测：kind: Service&& name: wordpress-mysql&& kind: PersistentVolumeClaim&& name: mysql-pv-claim&& mountPath: /var/lib/mysql
...
```shell
[root@master ~]# kubectl create secret generic mysql-pass --from-literal=password=000000
[root@master ~]# kubectl get secret
NAME                  TYPE                                  DATA   AGE
default-token-mmpph   kubernetes.io/service-account-token   3      29d
mysql-pass            Opaque                                1      9h
[root@master wordpress]# cat mysql-deployment.yaml 
[root@master wordpress]# kubectl apply -f mysql-deployment.yaml
```
...
编写yaml文件创建wordpress服务要求名称为wordpress标签为wordpress开放端口为80访问类型为NodePort,同时创建PersistentVolumeClaim名称为wp-pv-claim大小为2GB读写模式同时创建Deployment名称为wordpress使用wordpress:4.8-apache镜像同时指定数据库密码从名称为mysql-pass的secret获取设定数据库连接地址为wordpress-mysql，并将/var/www/html挂载到wordpress-persistent-storage中将编写的yaml文件以及kubectl get po -A返回结果以及



kubectl get svc返回结果黏贴至答题框。【5分】



评分检测：port: 80&& name: wordpress&&- image: wordpress:4.8-apache&& mountPath: /var/www/html
...
```yaml
[root@master wordpress]# cat wordpress-deployment.yaml 
apiVersion: v1
kind: Service
metadata:
  name: wordpress
...
[root@master wordpress]# kubectl apply -f wordpress-deployment.yaml
[root@master wordpress]# kubectl get svc
NAME              TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)        AGE
kubernetes        ClusterIP   10.96.0.1        <none>        443/TCP        7h57m
wordpress         NodePort    10.110.254.253   <none>        80:32272/TCP   5h51m
wordpress-mysql   ClusterIP   None             <none>        3306/TCP       5h51m
```
...
部署完成以后使用ping命令测试网页能否访问。

请将ping命令提交到答题框。【2分】

 

  ![img](./.图片存放/clip_image007.jpg)  

评分检测： ping http:/IP:32272

完整答案：

ping http:/IP:32272

\#在内网新建windows主机访问http:/IP:32272即可访问wordpress。
